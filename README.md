...
- **"Sign in to confirm you're not a bot" / downloads fail.** YouTube throttles datacenter IPs. Provide `YTDLP_COOKIES` (a `cookies.txt` from a logged-in throwaway account) and/or `YTDLP_PROXY`. The container attempts a best-effort yt-dlp update on start (and via the **Update yt-dlp** button in the UI) — it doesn't block startup and isn't guaranteed to succeed (e.g. offline, PyPI unreachable), so pull/rebuild the image for a guaranteed update. A failed trailer just retries next run.
