...
- **"Sign in to confirm you're not a bot" / age-gated / "Video unavailable" downloads fail.** YouTube throttles datacenter IPs and gates some trailers. Fix: export a **Netscape-format cookies.txt** (e.g. the *Get cookies.txt LOCALLY* browser extension while logged into youtube.com) and **paste it straight into `docker-compose.yml`** — uncomment the `YTDLP_COOKIES_TEXT: |` block and paste your lines under it (no separate file needed; the app writes it out internally and auto-repairs paste-mangled tabs). Then `docker compose up -d`. A proxy (`YTDLP_PROXY`) helps too. (You can also point `YTDLP_COOKIES` at a mounted file if you prefer.) The container attempts a best-effort yt-dlp update on start (and via the **Update yt-dlp** button in the UI) — it doesn't block startup and isn't guaranteed to succeed (e.g. offline, PyPI unreachable), so pull/rebuild the image for a guaranteed update. A failed trailer just retries next run.
